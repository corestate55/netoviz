...
class InterTpLink {
  ...
  skewType() {
    ...
    if (this.y1 === this.y2) {
      ...
      return 'vertical'
    } else if (this._onQuadrant24()) {
      return 'slash'
    } else if (this._onQuadrant13()) {
      return 'backslash'
    }
  }
  ...
  /**
   * Check skew type is backslash.
   * @returns {boolean} True if backslash.
   * @public
   */
  isSkewBackslash() {
    return this.skewType() === 'backslash'
  }
  ...
  _isSameSkew(otherLink, skewType) {
    return (
      this.skewType() === otherLink.skewType() && this.skewType() === skewType
    )
  }
