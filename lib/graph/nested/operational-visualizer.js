...
export default class OperationalNestedGraphVisualizer extends SingleNestedGraphVisualizer {
  ...
  setTpClassByPath(tpPath, tpClass) {
    this.selectTpCircleByPath(tpPath).classed(tpClass, true)
  }
  ...
  setLineClass(line, lineClass) {
    ...
    // link line
    if (line.type === 'tp-tp') {
      this.selectTpTpLineByPath(line.path).classed(lineClass, true)
    } else {
      this.selectSupportTpLineByPath(line.path).classed(lineClass, true)
    }
  }
  ...
  setLinkMouseHandler() {
    ...
    ;[this.selectTpTpLineByPath(), this.selectSupportTpLineByPath()].forEach(
      target => {
        target
          .on('click', lineClick)
  ...
  setSelectReady(node) {
    if (node.type === 'node') {
      this.selectNodeRectByPath(node.path).classed('select-ready', true)
    } else {
      this.selectTpCircleByPath(node.path).classed('select-ready', true)
    }
  }
  ...
  setTpMouseHandler() {
    ...
    ;[this.selectTpCircleByPath(), this.selectTpLabelByPath()].forEach(
      target => {
        target
  ...
  setNodeMouseHandler() {
    ...
    ;[this.selectNodeRectByPath(), this.selectNodeLabelByPath()].forEach(
      target => {
        target
          .on('click', nodeClick)
  ...
  _maxWHOnGrid(xy, i) {
    ...
    const selectAttribute = this.selectXY(
      xy,
      n => n.width,
      n => n.height
    )
