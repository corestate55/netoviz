 ...
 * @extends {SingleNestedGraphVisualizer}
 */
class OperationalNestedGraphVisualizer extends SingleNestedGraphVisualizer {
  /**
   * Find(-all) root node on grid [i, j].
   * @param {number} i - Ordinal number of xGrid. (-: for multiple select)
   * @param {number} j - Ordinal numbere of yGrid. (-1 for multiple select)
   * @returns {Array<GraphNode>} Root nodes on grid.
   * @private
   */
   ...
   * @param {string} [path] - Path of node. (All node rect when omitted.)
   ...
   * @param {number} i - Ordinal number of grid. (-1 for all lines)
   ...
   * @param {string} xy - Axis. (x/y)
   * @param {number} i - Ordinal number of grid. (-1 for all handles)
   * @returns {Selection} Grid circles.
   * @private
   */
  _selectGridHandle(xy, i) {
    if (i < 0) {
      // all grid-[xy]-handle circle
   ...
   * @param {string} xy - Axis. (x/y)
   * @param {number} i - Ordinal number of grid. (-1 for all labels)
   * @returns {Selection} Grid labels.
   * @private
   */
  _selectGridLabel(xy, i) {
    if (i < 0) {
      // all grid-[xy]-label text
   ...
   * @param {GraphNode} rootNode - Node to update.
   * @private
   */
   ...
   * @param {GraphNode} node - Node.
   * @returns {Array<GraphNode>} Term-points. (tp-type nodes)
   * @private
   */
  _tpsInNode(node) {
   ...
   * @param {GraphNode} tp - Term-point to update.
   * @private
   */
  _updateTpCirclePosition(tp) {
    this._selectTpCircleByPath(tp.path)
      .attr('cx', tp.cx)
      .attr('cy', tp.cy)
   ...
   * @param {GraphNode} node - Node.
   * @returns {Array<GraphNode>} Children of node.
   * @private
   */
   ...
   * @param {GraphNode} rootNode ^ Target root node.
   ...
   */
  _redrawLinkLines() {
    this.remakeLinkLines(this.graphData)
  ...
  /**
   * Set event hander of zooming to SVG.
   * @private
   */
  _setSVGZoom() {
   ...
   * @param {GraphNode} node - Target node.
   */
  setSelectReady(node) {
    if (node.type === 'node') {
   ...
   */
  _clearAllChecked() {
    this.svgGrp.selectAll('.checked').classed('checked', false)
  }
  ...
  _clearAllSelectReady() {
    this.svgGrp.selectAll('.select-ready').classed('select-ready', false)
  }
   ...
   * @param {GraphNode} d - Clicked Node.
   * @interface
   * @protected
   */
  nodeClickHook(d) {
  ...
  _setNodeMouseHandler() {
    ...
    const nodeMouseOver = d => {
      ...
      this.tooltip.enableTooltip(d, nodeClick)
    }
   ...
   * @param {NestedGraphData} graphData - Data of nested graph.
   * @public
   */
...
export default OperationalNestedGraphVisualizer
