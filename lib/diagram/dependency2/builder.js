...
class Dependency2DiagramBuilder extends MultilayerDiagramBase {
  ...
  constructor(restURIBase, grpcURIBase, width, height) {
    super('DEPENDENCY', restURIBase, grpcURIBase)
  ...
  makeAllDiagramElements(topologyData) {
    ...
    // change all `DependencyNodeData` objects
    // and `DependencyElement` data objects in topologyData (nodes/tps)
    // to `Dependency2NodeData` objects.
    this._rewriteNodeData()
     ...
     */
    this.nodesInNetworks = this._makeNodeDataListOfEachNetworks()
