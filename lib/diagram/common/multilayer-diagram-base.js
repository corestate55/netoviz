...
class MultilayerDiagramBase extends DiagramBase {
  constructor(graphName, restURIBase, grpcURIBase) {
    super()
    ...
    /** @const {string} REST API URI */
    this.restURIBase = restURIBase
    /** @const {string} gRPC API URI */
    this.grpcURIBase = grpcURIBase
  }
  ...
  restURI(graphName, params) {
    ...
    const baseUri =
      this.restURIBase + `/api/graph/${graphName}/${params.modelFile}`
