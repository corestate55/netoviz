...
class ForceSimulationDiagramVisualizer extends BaseContainer {
  ...
  constructor() {
    ...
    // NOTICE: Force-simulation visualizer does not inherit diagram-base,
    // since it controls multiple visualizer.
    // (Diagram-base is base-class of each single visualizer).
    // So, construct api uri independently.
   ...
   * @private
   */
  _getDiagramDataViaGRPC(jsonName, alert) {
   ...
   * @private
   */
  _getDiagramDataViaREST(jsonName, alert) {
  ...
  drawRfcTopologyData(jsonName, alert) {
    if (process.env.NODE_ENV === 'development') {
      this._getDiagramDataViaGRPC(jsonName, alert)
    } else {
      this._getDiagramDataViaREST(jsonName, alert)
    }
  }
