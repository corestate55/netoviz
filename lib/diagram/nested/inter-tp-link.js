...
import ForceSimulationLink from '~/server/graph/force-simulation/link'

/**
 * Inter term-point link. (tp-tp type link)
 * @extends {ForceSimulationLink}
 */
class InterTpLink extends ForceSimulationLink {
  /**
   * @param {NestedLinkData} link - Link of graph.
   * @param {NestedNodeData} sourceTp - Source term-point of the link. (tp-type node)
   * @param {NestedNodeData} targetTp - Destination term-point of the link. (tp-type node)
   */
  ...
  /**
   * Get SVG path as single line (for debugging)
   * @returns {string} SVG path
   */
  singleLine() {
  ...
  /**
   * Get SVG path as polyline (for debugging)
   * @returns {string} SVG path
   */
  polyline() {
    return this.represent4Points()
      .map((p, i) => {
