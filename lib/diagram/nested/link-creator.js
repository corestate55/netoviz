...
class InterTpLinkCreator {
  /**
   * @param {NestedTopologyData} topologyData - Topology (graph) data of nested graph.
   */
  constructor(topologyData) {
    ...
    this.links = topologyData.links.map((link) => {
      const sourceTp = findNodeByPath(topologyData.nodes, link.sourcePath)
      const targetTp = findNodeByPath(topologyData.nodes, link.targetPath)
   ...
   */
  _adjustLineOverlap() {
    const uniqOverlapIndex = new Set(
      this.links.map((link) => link.overlapIndex)
    )
