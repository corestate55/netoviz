 ...
 * @param {string} grpcURIBase - gRPC API URI.
 * @param {number} alertLimit - Number of alerts.
 * @returns {Promise<Array<AlertData>>} - Alerts.
 */
export const getAlertsViaGRPC = async (grpcURIBase, alertLimit) => {
  const response = await grpcClient(grpcURIBase).getAlerts(alertLimit)
  return response.toObject().alertsList
}

/**
 * Get alerts using REST.
 * @param {string} restURIBase - REST API URI.
 * @param {number} alertLimit - Number of alerts.
 * @returns {Promise<Array<AlertData>>} - Alerts.
 */
export const getAlertsViaREST = async (restURIBase, alertLimit) => {
  const response = await fetch(restURIBase + `/api/alert/${alertLimit}`)
